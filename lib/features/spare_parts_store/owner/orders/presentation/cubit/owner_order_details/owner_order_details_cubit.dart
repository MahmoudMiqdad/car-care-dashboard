// إدارة تفاصيل الطلب وإجراءات المالك عليه.
...
class OwnerOrderDetailsCubit extends Cubit<OwnerOrderDetailsState> {
  ...
  Future<void> _runAction(
    ...
    result.fold(
      ...
      (updatedOrder) => emit(
        OwnerOrderDetailsLoaded(updatedOrder, successMessage: successMsg),
      ),
    );
  }
  ...
  void clearSuccessMessage() {
    final current = state;
    if (current is OwnerOrderDetailsLoaded && current.successMessage != null) {
      emit(OwnerOrderDetailsLoaded(current.order));
    }
  }
}
