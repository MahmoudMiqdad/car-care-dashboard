...
class _EmptyStateWidgetState extends State<EmptyStateWidget>
  ...
  void initState() {
    ...
    _wheelAnim = Tween<double>(begin: 0, end: 2 * pi).animate(
      CurvedAnimation(parent: _wheelCtrl, curve: Curves.linear),
    );

    _roadAnim = Tween<double>(begin: 0, end: 60).animate(
      CurvedAnimation(parent: _roadCtrl, curve: Curves.linear),
    );
...
class _CarPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final roofPath = Path()
      ...
      ..quadraticBezierTo(
          carX + carW * 0.32, carY,
          carX + carW * 0.44, carY - carH * 0.04)
      ..lineTo(carX + carW * 0.68, carY - carH * 0.04)
      ..quadraticBezierTo(
          carX + carW * 0.78, carY,
          carX + carW * 0.88, carY + carH * 0.35)
      ..close();
    ...
    final windPath = Path()
      ...
      ..quadraticBezierTo(
          carX + carW * 0.34, carY + carH * 0.06,
          carX + carW * 0.45, carY + carH * 0.02)
      ..lineTo(carX + carW * 0.68, carY + carH * 0.02)
      ..quadraticBezierTo(
          carX + carW * 0.76, carY + carH * 0.06,
          carX + carW * 0.84, carY + carH * 0.33)
      ..close();
    canvas.drawPath(
      windPath,
    ...
    _drawDot(canvas, Offset(w * 0.85, h * 0.28), accent,
        (sin(t * 1.8) * 0.5 + 0.5));
    _drawDot(canvas, Offset(w * 0.12, h * 0.40), primary,
        (sin(t * 1.8 + 2.0) * 0.5 + 0.5));
    _drawDot(canvas, Offset(w * 0.90, h * 0.55), accent.withOpacity(0.7),
